...
const originalsArray_1 = __importDefault(
  require("../../utilities/originalsArray")
);
...
resizing.get("/", (req, res) => {
  ...
  const outFilePath =
    pathRoot + `\\images\\thumbnails\\${height}x${width}${fileName}`;
  if (
    `${ImagePath}${height}x${width}${fileName}` === outFilePath ||
    width == 0 ||
    height == 0
  ) {
    res.send("No need to resize").sendFile(outFilePath);
  } else if (
    !fileName ||
    !enteredWidth ||
    !enteredHeight ||
